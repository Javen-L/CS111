...
void shut_down(int exitcode) {
  ...
  if (!ssl) {
    if (SSL_connected) SSL_shutdown(ssl);
    SSL_free(ssl);
  }
...
void outputmsg(int ofd, char *message) {
  ...
  int rv_w = -1;
  if (ofd == sockfd) {
    rv_w = SSL_write(ssl, message, len);
  }
  else {
    rv_w = write(ofd, message, len);
  }
  if (rv_w == -1) {
...
int handle_command() {
  ...
  int readrv = SSL_read(ssl, eob, bufsize - offset);
  if (readrv <= 0) {
    fprintf(stderr, "Failed to read from SSL\n");
    shut_down(2);
  }
...
void set_up_connection() {
  ...
  if (SSL_set_fd(ssl,sockfd) != 1) {
    fprintf(stderr, "Cannot set fd for SSL\n");
    shut_down(2);
  }
